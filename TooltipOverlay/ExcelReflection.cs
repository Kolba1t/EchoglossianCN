...
internal static class ExcelReflection
{
    ...
    public static IEnumerable<decimal> ReadNumbersFromPropertiesNamedLike(object? row, string namePart)
    {
        if (row == null || string.IsNullOrWhiteSpace(namePart))
        {
            yield break;
        }
        ...
        {
            if (!prop.Name.Contains(namePart, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            if (TryConvertDecimal(raw, out var value))
            {
                yield return value;
                continue;
            }

            // Some generated sheet fields expose arrays/spans of primitive values.
            // Do not throw if the value cannot be enumerated; just skip it.
            if (raw is System.Collections.IEnumerable enumerable && raw is not string)
            {
                foreach (var item in enumerable)
                {
                    if (TryConvertDecimal(item, out value))
                    {
                        yield return value;
                    }
                }
    ...
    public static string ExtractReferenceText(object? value, params string[] textPropertyNames)
    {
        if (value == null)
        {
            return string.Empty;
        ...
        var candidates = new List<object?> { value };
        foreach (var propName in new[] { "Value", "Row", "Data", "Target", "Item" })
        {
            var nested = TryGetPropertyValue(value, propName);
            if (nested != null)
            {
        ...
        {
            foreach (var propName in textPropertyNames)
            {
                var text = ExtractTextProperty(candidate!, propName);
                if (IsRicherText(text, best))
                {
                    best = text;
                }
            }
    ...
    public static bool TryExtractReferenceRowId(object? value, out uint rowId)
    {
        rowId = 0;
        if (value == null)
        {
        ...
        foreach (var propName in new[] { "RowId", "Row", "Id", "ID" })
        {
            var raw = TryGetPropertyValue(value, propName);
            if (TryConvertUInt32(raw, out rowId) && rowId > 0)
            {
                return true;
            }
        }

        // RowRef-like objects sometimes expose the actual row through Value.
        var nested = TryGetPropertyValue(value, "Value");
        if (nested != null && !ReferenceEquals(nested, value))
        {
            return TryExtractReferenceRowId(nested, out rowId);
        }
    ...
    private static bool TryConvertUInt32(object? raw, out uint value)
    {
        value = 0;
        if (raw == null)
        {
